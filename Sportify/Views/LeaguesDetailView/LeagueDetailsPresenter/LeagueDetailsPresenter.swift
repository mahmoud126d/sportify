...
class LeagueDetailsPresenter{
    ...
    func fetchEvents(sport:String,leagueId:Int){
        ...
        networkManger.fetchData(sport: sport, requestType: requestType, leagueId: leagueId) { (result: Result<[FixtureDto], Error>) in
            switch result {
            case .success(let events):
                print(events.count)
                let latestEvents = events.filter { event in
                    guard let finalResult = event.eventFinalResult else { return false }
                    return finalResult != "-"
                }
                let upCommingEvents = events.filter { event in
                    guard let finalResult = event.eventFinalResult else { return false }
                    return finalResult == "-"
                }
    ...
    func fetchTeams(sport:String,leagueId:Int
        ...
        networkManger.fetchData(sport: sport, requestType:requestType,leagueId:
                    leagueId) { (result: Result<[TeamDto], Error>) in
            switch result {
